 ...
 */
public class BaiChuanMedia {
	private static Logger logger = Logger.getLogger(BaiChuanMedia.class);
...
//        configuration.setType(MediaConfiguration.TYPE_TOP);
        configuration.setAk(BaiChuan.MediaAccessKey);
        configuration.setSk(BaiChuan.MediaAccessKeySecret);
        configuration.setNamespace(BaiChuan.MediaNamespace);
...
//        File  file = new File( "C:\\Users\\dell\\Desktop\\图片2.png" );
//        System.out.println(upload( file    ,"load/20180629","test.png"         ));
////        blockUpload();
////        blockUploadCancel();
//    }
     ...
     * 用户直接上传多媒体文件
     */
    public static String upload(File file,String dir,String name) {
...
//            file.delete();
    ...
    public static void blockUpload() throws IOException {
        File contentFile = getFile("resources/image/image.jpg");
        ...
        if (!multipartInitResult.isSuccess()) {
        	...
        	logger.info(multipartInitResult.getData().geteTag());
        }
        ...
        for (int i = 1; i < partCount; i++) {
            ...
            if (!uploadResult.isSuccess()) {
            	...
            	logger.info("MultipartUpload failed. ErrorMsg: " + uploadResult.toString());
                return;
            }
    ...
    public static void blockUploadCancel() throws IOException {
        ...
        if (!multipartInitResult.isSuccess()) {
            // 初始化分片上传失败,直接退出
        	logger.info("MultipartInit failed. ErrorMsg: " + multipartInitResult.toString());
            return;
        } else {
            // 初始化成功,可以把ETag打印出来,ETag即各个片的MD5值
        	logger.info(multipartInitResult.getData().geteTag());
    ...
    private static File getFile(String path) {
        URL url = BaiChuanMedia.class.getClassLoader().getResource(path);
