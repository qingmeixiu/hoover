...
public class IdGenerator {
    ...
    private static Queue<Long> idsList = new LinkedList<Long>();
    
    public static synchronized IdGenerator getIntance() {
		if (idGenerator == null) {
			idGenerator = new  IdGenerator(1023);
		}
		return idGenerator;
	}
    
    public static String  getId() {
    	return IdGenerator.getIntance().nextId()+"";
    }
    // 10位进程ID标识  
    ...
    public synchronized long nextId() {  
    	...
    	if(idsList.size()>0) {
    		return idsList.poll();
    	      ...
    	      for(int i=0;i<500;i++) {//每次生成500个id
				        long ts = timeGen();  
...
////            set.add(ig.nextId());
//            set.add(IdGenerator.getIntance().nextId());
//        	;
//        }
